...
def build_brief(
    ...
    lines.extend(
        [
            ...
            f"- Run `python \"${{CLAUDE_PLUGIN_ROOT}}/skills/student-presentation-ppt/scripts/pptx_delivery_check.py\" --pptx <pptx> --notes <notes> [--preview <preview>] --package-report <package-report.json> --qa-manifest <manifest> --output \"{delivery_report_path}\" --strict --json` (add `--allow-missing-preview` when render was skipped).",
